...
- **Options** : **Telemetry** (activée/désactivée pour ce récepteur),
  **Reduced telemetry power 25mW** (au lieu des 100 mW habituels, option utile si
  des servos proches captent des interférences RF), **High PWM Speed**
  (rafraîchissement des servos à 7 ms au lieu de 18 ms, à activer après avoir
  vérifié que vos servos peuvent suivre), **Telemetry port** (S.Port/F.Port/FBUS), **SBUS**
  (16 ou 24 voies, à condition que tous les appareils SBUS connectés prennent
  en charge le SBUS-24 avant son activation) et **Channel Mapping** pour
